...
- PowerShell 5.1+ (scripts); Markdown (agent mode + templates) + PSScriptAnalyzer (script linting); `powershell-yaml` module (YAML read/write for `_categories.yaml` and `specs.yaml`); PowerShell built-in `ConvertFrom-Json` / `ConvertTo-Json` (003-transcript-to-spec)
- File system — `specs/<tier>/<category>/spec.md`, `specs/<tier>/_categories.yaml`, `specs/specs.yaml` (003-transcript-to-spec)
...
## Recent Changes
- 003-transcript-to-spec: Added PowerShell 5.1+ (scripts); Markdown (agent mode + templates) + PSScriptAnalyzer (script linting); `powershell-yaml` module (YAML read/write for `_categories.yaml` and `specs.yaml`); PowerShell built-in `ConvertFrom-Json` / `ConvertTo-Json`
